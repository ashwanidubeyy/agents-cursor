...
export type SubmitFormCallback = (
 ...
 *
 * Field handlers use the React Native `(name, value)` signature (a TextInput's
...
export const useForm = (
  ...
  const t = useCallback(
    (value: string) =>
      typeof translate === 'function' ? translate(value) : value,
    [translate],
  );

  const [state, setStateSchema] = useState<FormSchema>({ ...stateSchema });
  const [values, setValues] = useState<Record<string, any>>(
    get_prop_values(stateSchema, 'value'),
  );
  const [errors, setErrors] = useState<Record<string, string>>(
    get_prop_values(stateSchema, 'error'),
  );
  const [dirty, setDirty] = useState<Record<string, boolean>>(
    get_prop_values(stateSchema, ''),
  );
  ...
  const validateFormFields = useCallback(
    (name: string, value: any): string => {
      ...
      if (error === '' && is_object(field?.validator)) {
        const validator = field.validator as Validator;
  ...
  const setInitialErrorState = useCallback(() => {
    setErrors((prevState) =>
      Object.keys(prevState).reduce(
        (accumulator: Record<string, string>, name) => {
  ...
  const handleOnNumberChange = useCallback(
    (name: string, value: any) =>
      updateValue(name, value === '' ? '' : Number(value)),
    [updateValue],
  );

  const handleOnSelection = useCallback(
    (name: string, value: any) => updateValue(name, value),
    [updateValue],
  );

  const handleOnMultipleSelection = useCallback(
    (name: string, selectedValues: any[]) =>
      updateValue(name, selectedValues ?? []),
    [updateValue],
  );

  const handleOnCheckBoxChange = useCallback(
    (name: string, checked: boolean) => updateValue(name, Boolean(checked)),
    [updateValue],
  );

  const handleOnSwitch = useCallback(
    (name: string, value: boolean) => updateValue(name, Boolean(value)),
    [updateValue],
  );
  ...
  const handleOnSubmit = useCallback(
    (extraParams?: any) => {
      const nextErrors = Object.keys(state).reduce(
        (accumulator: Record<string, string>, name) => {
          accumulator[name] = validateFormFields(name, values?.[name]);
          return accumulator;
        },
  ...
  const setRequired = useCallback((fields: Record<string, boolean>) => {
  ...
  const setValidator = useCallback((name: string, validator: Validator) => {
    setStateSchema((prevState) => {
      if (!prevState[name]) {
        return prevState;
      }
  ...
  const handleNewSchema = useCallback((newSchema: FormSchema) => {
  ...
  const addUseFormFields = useCallback(
    ({
      keys = [],
      errorMessages = [],
      newSchemaFields = null,
    }: {
      keys?: string[];
      errorMessages?: string[];
      newSchemaFields?: FormSchema | null;
    }) => {
      const valuesToSet: Record<string, any> = {};
      const errorsToSet: Record<string, string> = {};
      const dirtyToSet: Record<string, boolean> = {};
  ...
  const removeUseFormFields = useCallback((keys: string[] = []) => {
    if (!keys.length) {
      return;
    }
    const omit = (prevState: Record<string, any>) => {
      const nextState = { ...prevState };
    ...
    setErrors(omit);
    setDirty(omit);
    setStateSchema(omit as (prev: FormSchema) => FormSchema);
  }, []);

 ...
 * working unchanged. React Native ships `fetch`, `XMLHttpRequest`, `FormData`
 * and `AbortController` globally, so this runs without any native module.
 */
 ...
 * upload progress. In React Native pass a `FormData` body that contains
 * `{ uri, name, type }` parts. Resolves with an axios-like response.
 */
...
/**
 * Base URL for the default client. Set it from your env/config layer
 * (e.g. react-native-config `Config.API_BASE_URL`) or replace this constant.
 */
const BASE_URL =

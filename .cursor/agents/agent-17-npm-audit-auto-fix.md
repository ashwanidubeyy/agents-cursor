---
name: agent-17-npm-audit-auto-fix
model: fast
---

# Agent 17: npm Audit Auto-Fix Agent (React Native)
...
**Output:** Fixed dependencies (when possible) + report at `.cursor/logs/vulnerability/npm-audit-auto-fix-{timestamp}.md` + user notification in chat.

**Related:** Agent 06 (Vulnerability Agent) scans and documents only — does **not** apply fixes. Agent 17 **does** apply fixes.
...
- **Hook trigger:** After `npm install`, `npm i`, or `npm ci`, the hook at `.cursor/hooks/npm-audit-after-install.js` runs `.cursor/scripts/npm-audit-auto-fix.js` automatically.
...
```bash
node .cursor/scripts/npm-audit-auto-fix.js --trigger agent --cwd <project-root>
```
...
8. Save report to `.cursor/logs/vulnerability/npm-audit-auto-fix-{timestamp}.md` (or `...-error-...` / `...-partial-...` on failure).
...
2. For each remaining advisory:
   - Check if a **major** upgrade is required (`isSemVerMajor: true` in audit) — evaluate React Native compatibility before applying.
...
Report: .cursor/logs/vulnerability/npm-audit-auto-fix-{timestamp}.md
...
The project hook is configured in `.cursor/hooks.json`:
...
```json
{
  ...
  "hooks": {
    "afterShellExecution": [
      {
        "command": "node .cursor/hooks/npm-audit-after-install.js",
...
**Port to another project:** See [docs/NPM-AUDIT-AUTO-FIX-SETUP.md](../../docs/NPM-AUDIT-AUTO-FIX-SETUP.md).
...
- ✅ Save report with error logs to `.cursor/logs/vulnerability/`

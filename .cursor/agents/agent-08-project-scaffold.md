...
- **Error pages (Connection Lost + Unauthorized)** — Install global offline and unauthorized error pages so every app shows **Connection Lost** when there is no internet and can navigate to **Unauthorized** on 401/403. Run `node .cursor/scripts/setup-error-pages.js` from the project root (or copy from `.cursor/setup/error-pages/`). This adds `ConnectionLost` and `Unauthorized` screens, `NetworkGate` in `Root.js`, NetInfo-based `useNetworkStatus`, `navigationRef`, `handleUnauthorized`, and `@react-native-community/netinfo`. See `.cursor/agents/agent-18-error-pages.md`.
- **Keyboard layouts (inputs + chat)** — Install cross-platform keyboard-aware layouts so forms and chat inputs stay above the keyboard on iOS and Android. Run `node .cursor/scripts/setup-keyboard-layout.js` from the project root. Adds `KeyboardAwareLayout`, `ChatKeyboardLayout`, `useKeyboardHeight`, `useKeyboardInsets`; patches `BaseScreen` `edges` prop. See `.cursor/agents/agent-20-keyboard-layout.md` and `.cursor/rules/keyboard-layout.mdc`.
...
- Save **`.cursor/logs/project-scaffold/project-scaffold-{app-name-kebab}-{timestamp}.md`** with: **dynamic** App name, Folder name used for CLI, project root path (outside workspace), **exact CLI command run** (community CLI, no deprecated `react-native init`; TypeScript by default), list of folders/files added (including the **`useForm` hook + `form-validators`**, the **`lib/fetch-client` HTTP client, axios-free**, and **error pages: Connection Lost + Unauthorized + NetworkGate + NetInfo**), next steps (`cd <path-to-project>`, `npm install`, `cd ios && pod install`).
...
- List project root path (sibling to workspace, e.g. `../<ProjectFolderName>/` or absolute path) and main additions (src/, Root, AppRouteConfig, constants, store, screens, widgets, **hooks/useForm + utility/form-validators**, **lib/fetch-client (axios-free)**, **error pages: ConnectionLost, Unauthorized, NetworkGate, NetInfo**).
...
- **Does:** **Always** create the project via the **React Native Community CLI** first (use `npx @react-native-community/cli init`; do not use deprecated `npx react-native init`); create the project **outside** the workspace (sibling folder); use **dynamic** App name from user input; then add folder structure and boilerplate in the CLI-created project — create **`.env`** only (no `.env.local` / `.env.example`); **including the schema-based `useForm` hook + `form-validators`** (TypeScript by default, JS fallback) so forms work out of the box, **the dependency-free `lib/fetch-client` HTTP client (axios-free)** so networking works out of the box, **error pages (Connection Lost + Unauthorized + NetworkGate + NetInfo via `setup-error-pages.js`)** so offline/unauthorized UX works out of the box, **and keyboard layouts (`setup-keyboard-layout.js`)** so inputs stay above the keyboard on iOS and Android; update/merge index.js, babel.config.js (aliases incl. `@hooks`/`@utility`/`@lib`), tsconfig.json (`paths`), package.json; save scaffold log.
